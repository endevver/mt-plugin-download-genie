...
sub asset_stats {
    ...
    my $code = sub {
        ...
        $row->{created_on_formatted} =
            format_ts( MT::App::CMS::LISTING_TIMESTAMP_FORMAT(), $ts, $blog, 
    ...
    my $epoch_hour  = timelocal(0,0,$hours,$day,$month,$year); # Downloads this hour
    my $epoch_day   = timelocal(0,0,0,$day,$month,$year); # Downloads today
    my $epoch_month = timelocal(0,0,0,1,$month,$year); # Downloads this month
    ...
    # An asset ID was not supplied, so load information for all assets.
        ...
        if ($terms{created_by}) {
            ...
            $params{summary_label} = 'all assets downloaded by ' 
                . $author->nickname . ' (<a href="' . $app->mt_uri 
                . '?__mode=view&_type=author&id=' . $author->id . '">' 
                . $author->name . '</a>)';
